...
import 'package:jadwal_kajian_new/services/interfaces/prayer_service_interface.dart';

class PrayerService implements PrayerServiceInterface {
  ...
  Future<Map<String, dynamic>> _fetchPrayerTimes({
    ...
    try {
      ...
      final url = Uri.parse(
          ...
          '?city=$cityParam&country=$countryParam&method=20');
  ...
  Future<Map<String, String>> getPrayerTimesMap() async {
    try {
      ...
      return {
        ...
        'Dhuha': _cleanTime(timings['Dhuha'] ?? '06:15'),
  ...
  Future<Map<String, String>> getNextPrayer() async {
    try {
      ...
      return {'name': 'Maghrib', 'time': '18:00', 'remaining': '1 jam 30 menit'};
    }
  }
  ...
  Future<Map<String, String>> getDates() async {
    try {
      ...
      final gregDay = gregorian['day'].toString();
      final gregMonthNum = int.tryParse(gregorian['month']['number'].toString()) ?? 1;
      final gregMonth = _getIndonesianMonth(gregMonthNum);
      final gregYear = gregorian['year'].toString();
      final gregWeekday = _getIndonesianWeekday(gregorian['weekday']['en'].toString());

      final hijri = date['hijri'];
      final hijriDay = hijri['day'].toString();
      final hijriMonth = hijri['month']['en'].toString();
